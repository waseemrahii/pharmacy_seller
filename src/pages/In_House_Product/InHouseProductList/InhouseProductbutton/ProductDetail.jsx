...
const ProductDetail = () => {
  ...
  const handleStatusChange = async (reviewId, currentStatus) => {
    ...
    try {
      await axios.put(`${ApiUrl}/api/products/${productId}/reviews/${reviewId}/status`, { status: newStatus });
...
const handleDeleteReview = async (reviewId) => {
    try {
      ...
      await axios.delete(`${ApiUrl}/api/products/${productId}/reviews/${reviewId}`);
  ...
  const thumbnailUrl = `${ApiUrl}/${productData.thumbnail.replace(/\\/g, '/')}`;
  const {
  thumbnail = '',
  images = [],
  name = 'No Name',
              ...
              <div className="d-flex flex-wrap align-items-center flex-sm-nowrap justify-content-between gap-3 min-h-50">
                <div className="d-flex flex-wrap gap-2 align-items-center">
                  {productData.images.map((imgUrl, index) => {
                    const fullImgUrl = `${ApiUrl}/${imgUrl.replace(/\\/g, '/')}`;
                    return (
