...
import { FaSearch, FaEye, FaEdit, FaTrashAlt, FaRecycle } from "react-icons/fa";
...
const SendNotification = () => {
  ...
  const handleResend = async (id) => {
    try {
      const response = await fetch(`http://localhost:3000/api/notification/${id}/increment`, {
        method: "PUT",
      });
      if (!response.ok) throw new Error("Failed to increment notification count");
      const updatedNotification = await response.json();
      setNotifications(notifications.map(notification => 
        notification._id === id ? updatedNotification : notification
      ));
      ...
      <div className="h-[80vh] w-full bg-white grid grid-cols-2 rounded-lg mt-3 px-10 py-8 gap-3">
        <div>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex justify-end items-end gap-3">
              <button type="reset" className="bg-[#EDEDED] rounded-md px-4 py-1 border border-gray-300">
                Reset
              </button>
              <button type="submit" className="rounded-md px-4 bg-green-400 text-white py-1 border border-green-300 hover:bg-green-600">
                Submit
              </button>
            </div>
          </form>
        ...
        <div className="flex flex-col gap-5">
          <div className="flex justify-center items-center px-8 py-5">
            <img
              src={uploadedImage || "https://6valley.6amtech.com/public/assets/back-end/img/image-place-holder.png"}
              alt="Uploaded"
              className="h-48 w-48 border rounded-sm border-gray-900"
            />
          </div>
          <div className="ms-4 flex flex-col">
            <label htmlFor="image-upload" className="text-md">Image (Ratio 1:1)</label>
            <div>
              <input
                type="file"
                id="image-upload"
                  ...
                  <form action="https://6valley.6amtech.com/admin/coupon/add" method="GET">
                    <div className="input-group input-group-merge input-group-custom">
                      ...
                      <input
                        ...
                        required
                      />
                      <button type="submit" className="btn bg-green-400 text-white border-green-400">
                        Search
                      </button>
                    </div>
                  </form>
                    ...
                    <tbody>
                      {notifications.map((notification) => (
                        <tr key={notification._id}>
                          ...
                          <td>
                            <img
                              src={notification.image || "https://6valley.6amtech.com/public/assets/back-end/img/image-place-holder.png"}
                              alt={notification.title}
                              className="h-12 w-12 border border-gray-200 rounded"
                            />
                          </td>
                          <td className="text-center">{notification.count}</td>
                          <td>
                            <button
                              onClick={() => handleStatusToggle(notification._id, notification.status)}
                              className={`px-4 py-1 rounded-md ${notification.status === "Active" ? "bg-green-400" : "bg-gray-400"} text-white`}
                            >
                              {notification.status}
                            </button>
                          </td>
